...
exports.getNotifications = async () => {
    ...
    const projects = (await Project.find({booking: true, events: {$gt: []}}, 'events timingClient timingUpp name work team K2 onair bookingBudget bookingType').lean().populate('events')).map(project => projectToBooking(project, true)).filter(project => !project.internal && !project.offtime && !project.rnd && project.events.length > 0 && project.manager);
...
exports.getProjectsFinal = async () => {
    ...
    const projectsData = (await Project.find({booking: true, deleted: null, paymentChecked: null}, 'events team name K2 bookingBudget work bookingType').lean().populate('events')).map(project => projectToBooking(project, true)).filter(project => !project.internal && !project.offtime && !project.rnd && project.confirmed);
