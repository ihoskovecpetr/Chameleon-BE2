 ...
 *
 * (C) 2016 Marc Harter, modified by MK 2018
 * MIT LICENCE
 */
 ...
 */
const Mail = exports.Mail = function(options) {
    options = options || {};
    ...
    this.frequency = options.frequency || 0; //in sec
...
Mail.prototype.log = function(level, msg, meta, callback) {
    ...
    if (this.formatter) {
        ...
        body = `${moment(now).format('HH:mm:ss')} [${level}]: ${msg}`;
    ...
    if(this.frequency) {
        if(!this.emailTimer && ((now - this.lastEmail) > (this.frequency * 1000))) {
            this.sendEmail(this.bodyArray);
            this.lastEmail = new Date();
            this.bodyArray = [];
            this.emailTimer = setTimeout(() => {
                self.sendEmail(this.bodyArray);
                this.lastEmail = new Date();
                this.bodyArray = [];
                this.emailTimer = null;
            }, this.frequency * 1000);
...
Mail.prototype.sendEmail = function(bodyArray) {
